...
The capability implemented and live-approved today is `SEARCH_TWEETS`: given a search query, fetch matching tweets, with pagination across multiple pages if requested. A second capability, `TWEET_BY_ID` (fetch one tweet by its ID, no pagination), is built end to end — capability contract, request builder, parser, worker dispatch, canonical storage — and its `operation_id`/`feature_bundle` now come from a real 2026-08-14 browser capture (`docs/deployment_runbook.md` documents the procedure used), so its recipe binding is `CANDIDATE`/`OBSERVED` rather than `DRAFT`/`INFERRED`. It still isn't `APPROVED` or live traffic-eligible: `validation_freshness` is `NEVER_VALIDATED` because it has never been run through the fixture/capture-replay validation pipeline (currently hardcoded to `SEARCH_TWEETS` — see `docs/TASKS.md`). Everything below describes `SEARCH_TWEETS`'s live path; §3.7 covers what `TWEET_BY_ID` reuses versus what's genuinely new per capability.
