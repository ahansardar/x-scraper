...
Date: 2026-08-14 (updated: added a second capability, `TWEET_BY_ID`, as a full vertical slice alongside `SEARCH_TWEETS` -- shares tweet-field parsing, worker execution, and canonical storage; its recipe binding is now CANDIDATE/OBSERVED after a real browser capture of X's `TweetResultByRestId` GraphQL operation ID, still pending fixture/capture-replay validation before approval)
...
The project is currently a production-oriented local vertical slice for `SEARCH_TWEETS`, with a second capability (`TWEET_BY_ID`) now built end to end and backed by a real captured GraphQL operation ID, but not yet approved for production traffic (its recipe has never been run through fixture/capture-replay validation).
...
- `TWEET_BY_ID` reuses `SEARCH_TWEETS`'s tweet-field extraction (`xprotocol/runtime/tweet_fields.py`), worker/canonical-store integration (its result is wrapped as a one-tweet, no-continuation page so the rest of `LocalWorker._process_delivery()` needs no capability-specific branching beyond building the request), and `AUTHORIZED_WEB_SESSION` auth/transaction/client profiles. Its `operation`/`feature_bundle` revisions carry a real browser-captured `operation_id` and feature set as of 2026-08-14 (`CANDIDATE`/`OBSERVED` in `protocol_releases/search_tweets.candidate.json`), while `parser`/`pagination` remain `DRAFT`/`INFERRED` (unvalidated against a real response). `validation_freshness` is `NEVER_VALIDATED` across the recipe -- it has never run through `protocol_validation.py`'s fixture/capture-replay pipeline, so this capability is not yet approved for production traffic.
...
2. `TWEET_BY_ID` is now a second capability (architecturally complete, recipe binding `CANDIDATE`/`OBSERVED` as of a real 2026-08-14 browser capture of `TweetResultByRestId`'s operation ID and feature bundle -- see `docs/deployment_runbook.md`'s "Capturing a New GraphQL Operation ID" section for the procedure used). It is still unapproved: `validation_freshness` is `NEVER_VALIDATED` because `protocol_validation.py`'s fixture/capture-replay pipeline is still hardcoded to `SEARCH_TWEETS` (see `docs/TASKS.md`). Capability-parameterizing that pipeline is the remaining blocker before `TWEET_BY_ID` -- or any further capability -- can be safely approved.
