...
- [~] Add the next capability vertical slice after `SEARCH_TWEETS` is fully release-governed. `TWEET_BY_ID` now has a full slice -- capability contract, operation builder, parser (sharing tweet-field extraction with `SEARCH_TWEETS` via `xprotocol/runtime/tweet_fields.py`), a degenerate `single_page` pagination strategy, worker dispatch, canonical ingest reuse, and tests -- but its recipe binding is `DRAFT`/`INFERRED` with a placeholder `operation_id` (`REPLACE_WITH_CAPTURED_TWEET_RESULT_BY_REST_ID_OPERATION_ID` in `protocol_releases/search_tweets.candidate.json`) because no live browser capture of X's real `TweetResultByRestId` GraphQL operation ID exists yet -- see `docs/deployment_runbook.md`'s "Capturing a New GraphQL Operation ID" section for the DevTools procedure. Not counted as done until that capture happens and the binding is fixture/capture-replay validated the way `SEARCH_TWEETS` is (see the fixture-pipeline gap below).
...
- [~] Only one of eleven spec-listed capabilities exists (spec §5): `SEARCH_TWEETS` is implemented; `TWEET_BY_ID` now has a full architectural slice but an unverified (placeholder) operation ID -- see the "next capability vertical slice" item above. `TWEET_REPLIES`, `TWEET_QUOTES`, `USER_LOOKUP`, `USER_TIMELINE`, `FOLLOWERS`, `FOLLOWING`, `LIST_TIMELINE`, `COMMUNITY_TIMELINE`, `MONITOR_QUERY`, `MONITOR_USER` remain fully unstarted.
